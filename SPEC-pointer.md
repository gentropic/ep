...
`@gcu/pointer` defines a fragment-based content addressing layer for browser-native tools. It specifies a grammar for pointer strings, an encoding for fragment-carried content, a set of resolution schemes for external content (GitHub, gist, Zenodo, rentry, generic CORS-open URLs), and an extension mechanism for additional schemes. The primary consumer is the Auditable shell, which uses it to open notebooks from any supported pointer type without requiring dedicated infrastructure. Other GCU shells (Arborist, Coreshed, GCU Works tools) may adopt it for equivalent use cases, and non-notebook consumers — QR-carried restaurant menus, doorbell ping pages, lost-and-found tags, conference reference cards — use the same resolver to ferry self-contained payloads through a static bootloader.

The spec is deliberately narrow. It is not a transport, not a storage system, not an authenticity or versioning system. It is a string-to-bytes resolver with a shared grammar, designed to run in a browser with no non-optional dependencies. Payload interpretation — what the resolved bytes *mean*, how renderers are selected, how multiple content types share a single bootloader — is a consumer concern, addressed by sibling specs such as `SPEC-void.md`.
...
- Define a stable string grammar such that a given pointer means the same thing forever.
...
- Not a storage system. Durability is inherited from whatever host the pointer references.
- Not an authenticity or integrity system. A layer above may verify content; this spec does not.
- Not a versioning system for the content itself. Reference schemes may carry version hints (`@ref`, `:version`) but this is host-specific, not part of the pointer semantics.
...
- Not a type system or renderer dispatcher. Resolved bytes are opaque to `@gcu/pointer`. Consumers needing to multiplex multiple payload kinds over a single bootloader (e.g., menu, doorbell, lost-and-found tag, and notebook payloads through one `/v#…` endpoint) layer a type-tag convention on top of the bytes returned by `resolve`. See `SPEC-void.md` for the canonical such layer.

### 2. Terminology

- **Shell** — a hosted application that uses `@gcu/pointer` to load content. Examples: Auditable, Arborist viewer.
...
- **Pointer** — a string of the form `scheme ":" body` identifying how to obtain content.
- **Fragment** — the `#...` portion of a URL. When a fragment carries a pointer, the shell reads it on load.
- **Loader** — code implementing the resolution of one scheme.
- **Dispatcher** — the glue that selects a loader based on pointer scheme.
- **Inline pointer** — a pointer whose scheme is `inline`, carrying content directly.
- **Reference pointer** — any non-`inline` scheme, resolving to external content.
...
A pointer is a string. Its ABNF is:
...
Pointers are case-sensitive. Scheme names are lowercase by convention; the dispatcher matches schemes case-sensitively and does not perform normalization.

A pointer appearing in a URL fragment is not further encoded: the fragment body is the pointer body directly. However, if the pointer contains characters that cannot appear in a URL fragment (controls, spaces, and characters outside the fragment production in RFC 3986), those characters must be percent-encoded. Loaders that accept URL-bearing bodies (e.g., `url:`) should expect percent-encoded input and decode accordingly.
...
"Required" means a conforming `@gcu/pointer` implementation MUST include a loader for the scheme. Shells consuming `@gcu/pointer` MAY disable individual loaders at integration time (§13).
...
Where `<shell-url>` is the absolute URL of a shell (e.g., `https://gentropic.org/auditable`) and `<pointer>` is a well-formed pointer per §3.

No part of the fragment other than the pointer is defined by this spec. Shells MAY use additional fragment syntax (for example, query-string-style parameters after a separator) for shell-specific state such as scroll position or active cell, but those MUST NOT collide with pointer syntax and SHOULD use a distinct leading character (e.g., `#<pointer>?cell=3`).

#### 4.1 Version negotiation

This spec's fragment layout is implicitly version 0. Future versions MAY introduce a version prefix of the form `v<n>:` *before* the pointer:

```
<shell-url>#v1:<pointer>
```
...
2. If `f` is empty, return *no pointer*.
...
The caller (the shell) is responsible for interpreting bytes as content. `@gcu/pointer` does not parse, validate, or transform content.

### 6. Inline form

The `inline:` scheme carries content directly in the pointer body.
...
The compact and long forms are semantically equivalent. `i:d<P>` MUST decode to the same bytes as `inline:deflate:<P>` given the same payload `P`. Implementations MUST accept both forms and MAY produce either; encoders targeting QR codes or other tight channels SHOULD prefer the compact form. The overhead reduction is 12 bytes per pointer (`inline:deflate:` vs `i:d`), which on typical text content at ~3× deflate ratio recovers roughly 36 bytes of source.
...
(The space after `r` / `d` is shown only for clarity in this document; the actual pointer concatenates the codec character directly with the base64url payload.)
...
SHA pinning is RECOMMENDED for share URLs intended to persist, because branch pointers and tag names can be rewritten in ways that silently change the content a share URL resolves to.
...
Zenodo supports both *concept DOIs* (which resolve to the latest version of a record) and *version DOIs* (which pin a specific deposit). When a pointer uses `zenodo:<record-id>`, the record ID may correspond to either; this is a property of how the record was deposited and is not encoded in the pointer. Callers concerned with pinning MUST use version-specific record IDs.
...
The `#file` suffix (note: `#` inside the pointer body, not a second URL fragment) is passed to the downstream loader as its file selector.
...
Rentry pastes are public. Implementations generating `rentry:` pointers SHOULD communicate this plainly to the user at the point of publication (§14).
...
interface ResolutionContext {
  ...
  // A function loaders MAY call to delegate a different pointer through
  // the dispatcher. Used by `doi:` to hand off to `zenodo:` after the
  // DOI redirect resolves.
  resolve(pointer: string): Promise<Uint8Array>;
...
- `ENOSCHEME` — pointer did not contain a `:`
...
Loaders MAY implement in-memory caching keyed on the pointer body. Cached entries SHOULD respect standard HTTP caching semantics returned by the target; in particular, responses with `Cache-Control: no-store` MUST NOT be cached.
...
interface Dispatcher {
  ...
  resolve(pointer: string, ctx?: Partial<ResolutionContext>): Promise<Uint8Array>;
}
```
...
`resolve` is the main entry point. It splits the pointer per §5, looks up the loader, constructs a full resolution context by merging supplied overrides with defaults (including a `resolve` back-reference for nested delegation), and invokes the loader.

A conforming `@gcu/pointer` package MUST export a function `createDispatcher(options?)` that returns a `Dispatcher` pre-registered with all required loaders (§3.1). Individual loaders MUST also be exported for standalone use and for tree-shaking by shells that want only a subset.

### 10. Encoding

This section describes the wire representation of inline pointers. Reference pointers have no additional encoding beyond the scheme-specific grammar in §7.
...
3. Construct pointer: `inline:deflate:<P>`.
...
```js
async function decodeInline(pointer) {
  const [, codec, payload] = pointer.match(/^inline:([^:]+):(.+)$/);
...
- surface an error to the user with the pointer string preserved so they can forward it to someone with a newer shell;
- attempt a fallback resolution (e.g., show a "try in updated shell" link).

Shells MUST NOT silently swallow pointer errors. A share URL that doesn't work is a worse user experience than a share URL that fails visibly.
...
- Use a namespaced scheme name (e.g., `acme-notebook:`) for public URLs, reserving short single-word scheme names for `@gcu/pointer` itself and well-known public infrastructure.
- Clearly document any additional CORS requirements, authentication, or rate limits.

Shells MAY curate which loaders are enabled. A shell that declines to register `rentry:`, for example, will fail `EUNKNOWN` on rentry pointers; this is intentional and allows shells to constrain their own attack surface and reduce dependency on external services they do not wish to depend on.
...
- Choose a `dict-id` identifying the dictionary content. The `dict-id` is opaque to `@gcu/pointer`; it is interpreted by the shell or consumer (e.g., `@gcu/void`) which maintains the dictionary registry.
...
Implementations MAY omit the `deflate-dict.*` codec entirely; a conforming implementation that omits it MUST reject such pointers with `EUNSUPPORTEDCODEC`. Implementations that target QR-bound content with locale-keyed dictionaries (the canonical case being `@gcu/void` menu payloads) SHOULD implement it.
...
The `@gcu/pointer` npm package has the following exports:

```
@gcu/pointer                -> createDispatcher, resolve (convenience),
                               encodeInline, decodeInline,
                               Dispatcher, Loader, ResolutionContext types
@gcu/pointer/loaders/inline -> inlineLoader
@gcu/pointer/loaders/url    -> urlLoader
@gcu/pointer/loaders/gh     -> ghLoader
@gcu/pointer/loaders/gist   -> gistLoader
@gcu/pointer/loaders/zenodo -> zenodoLoader
@gcu/pointer/loaders/doi    -> doiLoader
@gcu/pointer/loaders/rentry -> rentryLoader
@gcu/pointer/bundled        -> single-file ES module of all of the above
```
...
The total unminified size of `@gcu/pointer/bundled` SHOULD be under 20 KB. The spec expects a reference implementation to come in around 8–12 KB.

### 14. Share-side API (encoding content → pointers)

Generating pointers is the inverse operation to resolution. The package also exports a share API:

```ts
interface ShareOptions {
  // Target channel size budget, in bytes of encoded pointer. If the
...
type ShareResult =
  | { kind: "inline"; pointer: string; size: number }
...
`makeShare` encodes `content` with the `deflate` codec, measures the result, and either returns an inline pointer or a plan for the caller to fulfill (e.g., by POSTing to rentry). The plan-based approach keeps the actual publish side-effect out of `@gcu/pointer` — the package never performs writes, only reads.

Writes (e.g., rentry POST, gist creation) are left to the shell or to companion packages (`@gcu/pointer-publish`, if it ever exists).
...
The badge URL scheme is purely a shell convention — `@gcu/pointer` does not host badges. The recommended pattern is:

```
[![Open in <Shell>](<badge-svg-url>)](<shell-url>#<pointer>)
```
...
export function createDispatcher(init: DispatcherOptions = {}): Dispatcher {
  ...
  const resolve: Dispatcher["resolve"] = async (pointer, ctxOverride = {}) => {
...
A conforming implementation of `@gcu/pointer`:
...
- SHOULD implement `inline:deflate-dict.*` and `q:d.<dict-id>_*` codecs if the implementation is intended for use with `@gcu/void` or other consumers using dictionary-keyed content.
...
The remainder of the spec is informative. It describes how a shell (Auditable in the primary example) integrates `@gcu/pointer`, and gives guidance on UX for share generation.
...
3. **Pre-hydration UI.** The shell displays a loading indicator while resolution is in flight. For inline pointers this is typically under 50 ms; for reference pointers it depends on the target host and may be multiple seconds.
4. **Hydration.** On successful resolution, the shell parses the bytes as content and initializes its editor state from them.
5. **Post-hydration.** The shell becomes interactive. By default the hydrated content is editable. The shell MAY choose to show provenance information (which pointer was loaded) in its UI.
...
Auditable SHOULD default to Policy A for reference pointers (where live update would constantly rewrite a `gh:`/`gist:` pointer into an `inline:` one, silently destroying the original attribution) and MAY offer Policy B as a setting for inline-loaded sessions.

Shells MUST NOT modify the fragment of a reference pointer to a different reference pointer without explicit user action. The provenance of a share URL is part of its contract with the user.
...
4. **Point to an existing repo file** — generate a `gh:` pointer to content the user has already committed elsewhere.
5. **Cite by DOI** — for content already deposited in Zenodo or equivalent. Generate a `doi:` pointer.
...
Encryption protects against readers *of the pointer target* who do not have the key. It does not protect against readers who have the full share URL plus the key.

The typical case is: Alice wants to share an encrypted notebook with Bob via a link. Alice deposits ciphertext to a public host (rentry, gist). Alice generates a key and sends Bob the share URL, which carries the pointer (to ciphertext) plus the key (in fragment). Bob's browser fetches ciphertext, decrypts locally, hydrates. The server never sees plaintext.

Variants:

- **Key-in-fragment, ciphertext external.** Fragment = `v1:enc:<key>:<pointer-to-ciphertext>`. Ciphertext lives at a reference URL. Key lives in fragment, which is not server-logged.
- **Inline ciphertext, key-in-fragment.** Fragment = `v1:enc:<key>:<inline-ciphertext-pointer>`. Entirely self-contained. Useful when the caller wants zero external dependencies.
...
A versioned-fragment `v1:enc:...` grammar is one way to introduce encryption without disturbing v0. Another is a new scheme `enc:` that wraps an inner pointer:

```
enc:<key-material>:<wrapped-pointer>
```

Where the loader decrypts the inner pointer's bytes after resolution. The `enc:` form is simpler but less structured; the `v1:enc:` form allows fuller key-agreement semantics.

Choice is deferred to the encryption spec (whenever it's written). The v0 grammar in §3 explicitly reserves the behavior that a pointer scheme `enc` is allocatable for this purpose.

#### 21.3 What v0 gets right for future encryption

- Fragment is the key-carrying channel: fragments are not server-logged, so a key in the fragment is not leaked to the pointer target.
- Loaders return bytes, not parsed content: an encryption layer can wrap any loader without the loader knowing about it.
- Dispatcher delegation (`ctx.resolve`): an `enc:` loader can resolve the inner pointer through the dispatcher, fetching ciphertext bytes, then decrypt and return plaintext bytes.
...
A pointer in a fragment is therefore *not secret*. Treat fragment contents as public-adjacent. Encryption (§21) is the mechanism for actual secrecy.

#### 22.2 Malicious content

`@gcu/pointer` returns bytes. What the shell does with those bytes is the shell's responsibility. For Auditable specifically, the content may contain executable code (js cells, adder cells) that runs in the shell's origin with full access to shell APIs. Hydrating from an arbitrary pointer is equivalent in trust terms to opening a notebook file sent by an untrusted party.

Shells SHOULD display the pointer's provenance prominently (host, path) during pre-hydration, especially for reference pointers, and SHOULD provide the user a chance to inspect before executing.

Shells MAY sandbox untrusted hydration (iframe, Web Worker) depending on their use case. This is entirely outside `@gcu/pointer`'s concern.

#### 22.3 SSRF and host abuse

Browser `fetch` is subject to the same-origin policy and CORS. A malicious pointer cannot trigger requests to hosts that don't opt into CORS. This constrains what a hostile share URL can exfiltrate: at most, requests to CORS-open public hosts, none of which carry the user's cookies (since all fetches are cross-origin and credentialless by default in `@gcu/pointer` loaders).
...
Timing attacks against the fact that a pointer loaded successfully vs. failed are possible in theory (the attacker knows the user opened a share link because the attacker's server observes a fetch). In practice this only leaks which reference pointer was opened, which is visible in browser history anyway.

#### 22.4 Rate limit exhaustion

A malicious share URL could cause many fetches against a single third-party host. This is a denial-of-service against the host, not against the user. `@gcu/pointer` does not attempt to rate-limit at the client; hosts are expected to handle this themselves (GitHub API has rate limits, Zenodo has rate limits, etc.).

Shells that embed `@gcu/pointer` as a library (e.g., bulk notebook importers) SHOULD implement polite-crawling conventions against the reference hosts: serial rather than parallel requests when iterating, backoff on 429 responses, etc.
...
- **Host-agnostic** — the pointer namespace is not coupled to any one source host
...
For Auditable's `.txt` source form, GitHub Linguist registration is tracked separately from this spec. If and when a dedicated extension for Auditable notebooks is registered (`.auditable`, `.adt`, or similar), this spec's grammar is unaffected; content is opaque to pointers.
...
Pointers (all decode to the same content bytes):
...
Pointer: `gh:endarthur/auditable@main:README.md`
...
- **v0.1** (2026-05-16) — Initial draft. Defines pointer grammar (`SCHEME:BODY` in URL fragment), resolution algorithm, dispatcher API, and loader interface. Three inline schemes: `inline:` (long form, base64url), `i:` (compact form, base64url, ~12 bytes less framing), `q:` (QR-optimized, base45, ~22% denser in QR bit cost, the only inline form supporting dictionary-keyed deflate). Six reference schemes: `url`, `gh`, `gist`, `zenodo`, `doi`, `rentry`. Three codecs: `raw`, `deflate` (raw, RFC 1951, no zlib wrapper), `brotli` (OPTIONAL, RFC 7932); plus `deflate-dict.<dict-id>` for dictionary-keyed deflate (REQUIRED to implement for consumers like `@gcu/void`, with pako as the polyfill until native browser support). All test vectors in Appendix D verified round-trip in Python 3 reference encoder. Encryption integration surface deferred (§21) but worked example in §21.4 confirms grammar accommodates the anyone-can-send / only-owner-can-read case (sealed-box envelope, QR-doorbell). Payload type discrimination and renderer dispatch are explicit non-goals; see `SPEC-void.md` for the canonical dispatch layer.
...
- **Signing and verification.** Could be added as a wrapping scheme (`sig:<public-key>:<sig>:<inner-pointer>`). Non-trivial to get right; left for a dedicated spec.
- **Content-addressed identity.** A `cid:<hash>:<inner-pointer>` wrapper would let the dispatcher verify bytes against a hash post-fetch. Useful for tamper detection. Left for future work.
- **Search/discovery.** There is no registry of pointers, no federation, no listing. A pointer is only obtainable from whoever shared it with you.
- **Garbage collection.** Reference pointers break when their target is deleted; this spec does not mitigate. Shells MAY cache aggressively, but cache eviction is not specified.
...
- **Payload type discrimination and renderer dispatch.** A bootloader serving multiple payload kinds (menus, doorbells, notebooks, lost-and-found tags, conference references) needs a way to inspect resolved bytes and pick a renderer. This is a consumer concern; the canonical layer is `@gcu/void`. See `SPEC-void.md`.
