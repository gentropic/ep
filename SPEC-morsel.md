...
`@gcu/morsel` defines a fragment-based content addressing layer for browser-native tools. It specifies a grammar for morsel strings, an encoding for fragment-carried content, a set of resolution schemes for external content (GitHub, gist, Zenodo, rentry, generic CORS-open URLs), and an extension mechanism for additional schemes. The primary consumer is the Auditable shell, which uses it to open notebooks from any supported morsel type without requiring dedicated infrastructure. Other GCU shells (Arborist, Coreshed, GCU Works tools) may adopt it for equivalent use cases, and non-notebook consumers — QR-carried restaurant menus, doorbell ping pages, lost-and-found tags, conference reference cards — use the same resolver to ferry self-contained payloads through a static bootloader.

The spec is deliberately narrow. It is not a transport, not a storage system, not an authenticity or versioning system. It is a string-to-bytes resolver with a shared grammar, designed to run in a browser with no non-optional dependencies. Payload interpretation — what the resolved bytes *mean*, how renderers are selected, how multiple content types share a single bootloader — is a consumer concern, addressed by sibling specs such as `SPEC-kiosk.md`.
...
- Define a stable string grammar such that a given morsel means the same thing forever.
...
- Not a storage system. Durability is inherited from whatever host the morsel references.
- Not an authenticity or integrity system. A layer above may verify content; this spec does not.
- Not a versioning system for the content itself. Reference schemes may carry version hints (`@ref`, `:version`) but this is host-specific, not part of the morsel semantics.
...
- Not a type system or renderer dispatcher. Resolved bytes are opaque to `@gcu/morsel`. Consumers needing to multiplex multiple payload kinds over a single bootloader (e.g., menu, doorbell, lost-and-found tag, and notebook payloads through one `/k#…` endpoint) layer a type-tag convention on top of the bytes returned by `resolve`. See `SPEC-kiosk.md` for the canonical such layer.

### 2. Terminology

- **Shell** — a hosted application that uses `@gcu/morsel` to load content. Examples: Auditable, Arborist viewer.
...
- **Morsel** — a string of the form `scheme ":" body` identifying how to obtain content.
- **Fragment** — the `#...` portion of a URL. When a fragment carries a morsel, the shell reads it on load.
- **Loader** — code implementing the resolution of one scheme.
- **Dispatcher** — the glue that selects a loader based on morsel scheme.
- **Inline morsel** — a morsel whose scheme is `inline`, carrying content directly.
- **Reference morsel** — any non-`inline` scheme, resolving to external content.
...
A morsel is a string. Its ABNF is:
...
Morsels are case-sensitive. Scheme names are lowercase by convention; the dispatcher matches schemes case-sensitively and does not perform normalization.

A morsel appearing in a URL fragment is not further encoded: the fragment body is the morsel body directly. However, if the morsel contains characters that cannot appear in a URL fragment (controls, spaces, and characters outside the fragment production in RFC 3986), those characters must be percent-encoded. Loaders that accept URL-bearing bodies (e.g., `url:`) should expect percent-encoded input and decode accordingly.
...
"Required" means a conforming `@gcu/morsel` implementation MUST include a loader for the scheme. Shells consuming `@gcu/morsel` MAY disable individual loaders at integration time (§13).
...
Where `<shell-url>` is the absolute URL of a shell (e.g., `https://gentropic.org/auditable`) and `<morsel>` is a well-formed morsel per §3.

No part of the fragment other than the morsel is defined by this spec. Shells MAY use additional fragment syntax (for example, query-string-style parameters after a separator) for shell-specific state such as scroll position or active cell, but those MUST NOT collide with morsel syntax and SHOULD use a distinct leading character (e.g., `#<morsel>?cell=3`).

#### 4.1 Version negotiation

This spec's fragment layout is implicitly version 0. Future versions MAY introduce a version prefix of the form `v<n>:` *before* the morsel:
...
2. If `f` is empty, return *no morsel*.
...
The caller (the shell) is responsible for interpreting bytes as content. `@gcu/morsel` does not parse, validate, or transform content.

### 6. Inline form

The `inline:` scheme carries content directly in the morsel body.
...
The compact and long forms are semantically equivalent. `i:d<P>` MUST decode to the same bytes as `inline:deflate:<P>` given the same payload `P`. Implementations MUST accept both forms and MAY produce either; encoders targeting QR codes or other tight channels SHOULD prefer the compact form. The overhead reduction is 12 bytes per morsel (`inline:deflate:` vs `i:d`), which on typical text content at ~3× deflate ratio recovers roughly 36 bytes of source.
...
(The space after `r` / `d` is shown only for clarity in this document; the actual morsel concatenates the codec character directly with the base64url payload.)
...
(Spaces shown for readability; actual morsels concatenate without spaces.)

##### 6.4.1 Carrying a `q:` morsel in a URL fragment
...
When a `q:` morsel is placed in a URL fragment, an encoder MUST escape exactly these two characters, and a decoder MUST reverse exactly this escaping:
...
This escaping applies only to the URL-fragment transport. The morsel body itself (what a `resolve` implementation receives, and the bytes a QR encodes in alphanumeric mode) is the raw base45 with no escaping. A QR encoder MAY emit the raw base45 (including the literal space) directly in an alphanumeric segment; the escaping is required only where the morsel travels as URL text. `i:` and `inline:` (base64url) payloads contain neither character, so this transform is a no-op for them and MAY be applied uniformly to any morsel.

`q:` is semantically related to `i:` and the long-form `inline:` but is NOT byte-equivalent: the base encoding differs. The same content produces three valid morsels — `inline:deflate:<base64url>`, `i:d<base64url>`, `q:d<base45>` — that all decode to the same bytes. Encoders SHOULD prefer:
...
SHA pinning is RECOMMENDED for share URLs intended to persist, because branch morsels and tag names can be rewritten in ways that silently change the content a share URL resolves to.
...
Zenodo supports both *concept DOIs* (which resolve to the latest version of a record) and *version DOIs* (which pin a specific deposit). When a morsel uses `zenodo:<record-id>`, the record ID may correspond to either; this is a property of how the record was deposited and is not encoded in the morsel. Callers concerned with pinning MUST use version-specific record IDs.
...
The `#file` suffix (note: `#` inside the morsel body, not a second URL fragment) is passed to the downstream loader as its file selector.
...
Rentry pastes are public. Implementations generating `rentry:` morsels SHOULD communicate this plainly to the user at the point of publication (§14).
...
interface ResolutionContext {
  ...
  // A function loaders MAY call to delegate a different morsel through
  // the dispatcher. Used by `doi:` to hand off to `zenodo:` after the
  // DOI redirect resolves.
  resolve(morsel: string): Promise<Uint8Array>;
...
- `ENOSCHEME` — morsel did not contain a `:`
...
Loaders MAY implement in-memory caching keyed on the morsel body. Cached entries SHOULD respect standard HTTP caching semantics returned by the target; in particular, responses with `Cache-Control: no-store` MUST NOT be cached.
...
interface Dispatcher {
  ...
  resolve(morsel: string, ctx?: Partial<ResolutionContext>): Promise<Uint8Array>;
}
```
...
`resolve` is the main entry point. It splits the morsel per §5, looks up the loader, constructs a full resolution context by merging supplied overrides with defaults (including a `resolve` back-reference for nested delegation), and invokes the loader.

A conforming `@gcu/morsel` package MUST export a function `createDispatcher(options?)` that returns a `Dispatcher` pre-registered with all required loaders (§3.1). Individual loaders MUST also be exported for standalone use and for tree-shaking by shells that want only a subset.

### 10. Encoding

This section describes the wire representation of inline morsels. Reference morsels have no additional encoding beyond the scheme-specific grammar in §7.
...
3. Construct morsel: `inline:deflate:<P>`.
...
```js
async function decodeInline(morsel) {
  const [, codec, payload] = morsel.match(/^inline:([^:]+):(.+)$/);
...
- surface an error to the user with the morsel string preserved so they can forward it to someone with a newer shell;
- attempt a fallback resolution (e.g., show a "try in updated shell" link).

Shells MUST NOT silently swallow morsel errors. A share URL that doesn't work is a worse user experience than a share URL that fails visibly.
...
- Use a namespaced scheme name (e.g., `acme-notebook:`) for public URLs, reserving short single-word scheme names for `@gcu/morsel` itself and well-known public infrastructure.
- Clearly document any additional CORS requirements, authentication, or rate limits.

Shells MAY curate which loaders are enabled. A shell that declines to register `rentry:`, for example, will fail `EUNKNOWN` on rentry morsels; this is intentional and allows shells to constrain their own attack surface and reduce dependency on external services they do not wish to depend on.
...
- Choose a `dict-id` identifying the dictionary content. The `dict-id` is opaque to `@gcu/morsel`; it is interpreted by the shell or consumer (e.g., `@gcu/kiosk`) which maintains the dictionary registry.
...
Implementations MAY omit the `deflate-dict.*` codec entirely; a conforming implementation that omits it MUST reject such morsels with `EUNSUPPORTEDCODEC`. Implementations that target QR-bound content with locale-keyed dictionaries (the canonical case being `@gcu/kiosk` menu payloads) SHOULD implement it.
...
The `@gcu/morsel` npm package has the following exports:

```
@gcu/morsel                -> createDispatcher, resolve (convenience),
                               encodeInline, decodeInline,
                               Dispatcher, Loader, ResolutionContext types
@gcu/morsel/loaders/inline -> inlineLoader
@gcu/morsel/loaders/url    -> urlLoader
@gcu/morsel/loaders/gh     -> ghLoader
@gcu/morsel/loaders/gist   -> gistLoader
@gcu/morsel/loaders/zenodo -> zenodoLoader
@gcu/morsel/loaders/doi    -> doiLoader
@gcu/morsel/loaders/rentry -> rentryLoader
@gcu/morsel/bundled        -> single-file ES module of all of the above
```
...
The total unminified size of `@gcu/morsel/bundled` SHOULD be under 20 KB. The spec expects a reference implementation to come in around 8–12 KB.

### 14. Share-side API (encoding content → morsels)

Generating morsels is the inverse operation to resolution. The package also exports a share API:

```ts
interface ShareOptions {
  // Target channel size budget, in bytes of encoded morsel. If the
...
type ShareResult =
  | { kind: "inline"; morsel: string; size: number }
...
`makeShare` encodes `content` with the `deflate` codec, measures the result, and either returns an inline morsel or a plan for the caller to fulfill (e.g., by POSTing to rentry). The plan-based approach keeps the actual publish side-effect out of `@gcu/morsel` — the package never performs writes, only reads.

Writes (e.g., rentry POST, gist creation) are left to the shell or to companion packages (`@gcu/morsel-publish`, if it ever exists).
...
The badge URL scheme is purely a shell convention — `@gcu/morsel` does not host badges. The recommended pattern is:

```
[![Open in <Shell>](<badge-svg-url>)](<shell-url>#<morsel>)
```
...
export function createDispatcher(init: DispatcherOptions = {}): Dispatcher {
  ...
  const resolve: Dispatcher["resolve"] = async (morsel, ctxOverride = {}) => {
    const trimmed = morsel.startsWith("#") ? morsel.slice(1) : morsel;
...
A conforming implementation of `@gcu/morsel`:
...
- SHOULD implement `inline:deflate-dict.*` and `q:d.<dict-id>_*` codecs if the implementation is intended for use with `@gcu/kiosk` or other consumers using dictionary-keyed content.
...
The remainder of the spec is informative. It describes how a shell (Auditable in the primary example) integrates `@gcu/morsel`, and gives guidance on UX for share generation.
...
3. **Pre-hydration UI.** The shell displays a loading indicator while resolution is in flight. For inline morsels this is typically under 50 ms; for reference morsels it depends on the target host and may be multiple seconds.
4. **Hydration.** On successful resolution, the shell parses the bytes as content and initializes its editor state from them.
5. **Post-hydration.** The shell becomes interactive. By default the hydrated content is editable. The shell MAY choose to show provenance information (which morsel was loaded) in its UI.
...
Auditable SHOULD default to Policy A for reference morsels (where live update would constantly rewrite a `gh:`/`gist:` morsel into an `inline:` one, silently destroying the original attribution) and MAY offer Policy B as a setting for inline-loaded sessions.

Shells MUST NOT modify the fragment of a reference morsel to a different reference morsel without explicit user action. The provenance of a share URL is part of its contract with the user.
...
4. **Point to an existing repo file** — generate a `gh:` morsel to content the user has already committed elsewhere.
5. **Cite by DOI** — for content already deposited in Zenodo or equivalent. Generate a `doi:` morsel.
...
Encryption protects against readers *of the morsel target* who do not have the key. It does not protect against readers who have the full share URL plus the key.

The typical case is: Alice wants to share an encrypted notebook with Bob via a link. Alice deposits ciphertext to a public host (rentry, gist). Alice generates a key and sends Bob the share URL, which carries the morsel (to ciphertext) plus the key (in fragment). Bob's browser fetches ciphertext, decrypts locally, hydrates. The server never sees plaintext.

Variants:

- **Key-in-fragment, ciphertext external.** Fragment = `v1:enc:<key>:<morsel-to-ciphertext>`. Ciphertext lives at a reference URL. Key lives in fragment, which is not server-logged.
- **Inline ciphertext, key-in-fragment.** Fragment = `v1:enc:<key>:<inline-ciphertext-morsel>`. Entirely self-contained. Useful when the caller wants zero external dependencies.
...
A versioned-fragment `v1:enc:...` grammar is one way to introduce encryption without disturbing v0. Another is a new scheme `enc:` that wraps an inner morsel:

```
enc:<key-material>:<wrapped-morsel>
```

Where the loader decrypts the inner morsel's bytes after resolution. The `enc:` form is simpler but less structured; the `v1:enc:` form allows fuller key-agreement semantics.

Choice is deferred to the encryption spec (whenever it's written). The v0 grammar in §3 explicitly reserves the behavior that a morsel scheme `enc` is allocatable for this purpose.

#### 21.3 What v0 gets right for future encryption

- Fragment is the key-carrying channel: fragments are not server-logged, so a key in the fragment is not leaked to the morsel target.
- Loaders return bytes, not parsed content: an encryption layer can wrap any loader without the loader knowing about it.
- Dispatcher delegation (`ctx.resolve`): an `enc:` loader can resolve the inner morsel through the dispatcher, fetching ciphertext bytes, then decrypt and return plaintext bytes.
...
A morsel in a fragment is therefore *not secret*. Treat fragment contents as public-adjacent. Encryption (§21) is the mechanism for actual secrecy.

#### 22.2 Malicious content

`@gcu/morsel` returns bytes. What the shell does with those bytes is the shell's responsibility. For Auditable specifically, the content may contain executable code (js cells, adder cells) that runs in the shell's origin with full access to shell APIs. Hydrating from an arbitrary morsel is equivalent in trust terms to opening a notebook file sent by an untrusted party.

Shells SHOULD display the morsel's provenance prominently (host, path) during pre-hydration, especially for reference morsels, and SHOULD provide the user a chance to inspect before executing.

Shells MAY sandbox untrusted hydration (iframe, Web Worker) depending on their use case. This is entirely outside `@gcu/morsel`'s concern.

#### 22.3 SSRF and host abuse

Browser `fetch` is subject to the same-origin policy and CORS. A malicious morsel cannot trigger requests to hosts that don't opt into CORS. This constrains what a hostile share URL can exfiltrate: at most, requests to CORS-open public hosts, none of which carry the user's cookies (since all fetches are cross-origin and credentialless by default in `@gcu/morsel` loaders).
...
Timing attacks against the fact that a morsel loaded successfully vs. failed are possible in theory (the attacker knows the user opened a share link because the attacker's server observes a fetch). In practice this only leaks which reference morsel was opened, which is visible in browser history anyway.

#### 22.4 Rate limit exhaustion

A malicious share URL could cause many fetches against a single third-party host. This is a denial-of-service against the host, not against the user. `@gcu/morsel` does not attempt to rate-limit at the client; hosts are expected to handle this themselves (GitHub API has rate limits, Zenodo has rate limits, etc.).

Shells that embed `@gcu/morsel` as a library (e.g., bulk notebook importers) SHOULD implement polite-crawling conventions against the reference hosts: serial rather than parallel requests when iterating, backoff on 429 responses, etc.
...
- **Host-agnostic** — the morsel namespace is not coupled to any one source host
...
For Auditable's `.txt` source form, GitHub Linguist registration is tracked separately from this spec. If and when a dedicated extension for Auditable notebooks is registered (`.auditable`, `.adt`, or similar), this spec's grammar is unaffected; content is opaque to morsels.
...
Morsels (all decode to the same content bytes):
...
Morsel: `gh:endarthur/auditable@main:README.md`
...
- **v0.1** (2026-05-16) — Initial draft. Defines morsel grammar (`SCHEME:BODY` in URL fragment), resolution algorithm, dispatcher API, and loader interface. Three inline schemes: `inline:` (long form, base64url), `i:` (compact form, base64url, ~12 bytes less framing), `q:` (QR-optimized, base45, ~22% denser in QR bit cost, the only inline form supporting dictionary-keyed deflate). Six reference schemes: `url`, `gh`, `gist`, `zenodo`, `doi`, `rentry`. Three codecs: `raw`, `deflate` (raw, RFC 1951, no zlib wrapper), `brotli` (OPTIONAL, RFC 7932); plus `deflate-dict.<dict-id>` for dictionary-keyed deflate (REQUIRED to implement for consumers like `@gcu/kiosk`, with pako as the polyfill until native browser support). All test vectors in Appendix D verified round-trip in Python 3 reference encoder. Encryption integration surface deferred (§21) but worked example in §21.4 confirms grammar accommodates the anyone-can-send / only-owner-can-read case (sealed-box envelope, QR-doorbell). Payload type discrimination and renderer dispatch are explicit non-goals; see `SPEC-kiosk.md` for the canonical dispatch layer.
...
- **Signing and verification.** Could be added as a wrapping scheme (`sig:<public-key>:<sig>:<inner-morsel>`). Non-trivial to get right; left for a dedicated spec.
- **Content-addressed identity.** A `cid:<hash>:<inner-morsel>` wrapper would let the dispatcher verify bytes against a hash post-fetch. Useful for tamper detection. Left for future work.
- **Search/discovery.** There is no registry of morsels, no federation, no listing. A morsel is only obtainable from whoever shared it with you.
- **Garbage collection.** Reference morsels break when their target is deleted; this spec does not mitigate. Shells MAY cache aggressively, but cache eviction is not specified.
...
- **Payload type discrimination and renderer dispatch.** A bootloader serving multiple payload kinds (menus, doorbells, notebooks, lost-and-found tags, conference references) needs a way to inspect resolved bytes and pick a renderer. This is a consumer concern; the canonical layer is `@gcu/kiosk`. See `SPEC-kiosk.md`.
