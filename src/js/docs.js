...
export const DOCS = {
  ...
  stereonet:        { signature: 'stereonet() -> Plot', description: 'Empty equal-area stereonet. Layer features with `with_planes` / `with_lines` / `with_poles`; set title with `with_title`. A bare expression whose value is a Plot auto-renders inline. Use the `|>` pipe for the fluent form.', example: 'stereonet()\n  |> with_planes(faults.dd, faults.dip)\n  |> with_lines(slip.t, slip.p)\n  |> with_title("Combined")' },
